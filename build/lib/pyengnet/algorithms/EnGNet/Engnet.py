...
class Engnet:
    ...
    def __GPUmethod(dataset, saveComplete = False, numGpus = 1, CC = 61):
        os.system('cd pyengnet/algorithms/EnGNet && make clean')
        os.system('cd pyengnet/algorithms/EnGNet && make CC="'+str(CC)+'"')
        os.system('./pyengnet/algorithms/EnGNet/cuEnGNet --data '+str(dataset.filePath)+' --cors '+str(dataset.spearman_threshold)+' --cork '+str(dataset.kendall_threshold)+' --corn '+str(dataset.nmi_threshold)+' --gpu '+str(numGpus)+' --out')
